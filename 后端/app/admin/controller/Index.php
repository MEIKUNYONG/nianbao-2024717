...
class Index extends Admin {
	...
	private function getCardData(){
        ...
        $tuis = [];
        $tuis['refund_time']=['between',[$start,$end]];
        $tuis['is_pay']=1;
        $tuis['is_refund']=1;
        
        $tuiyue = OrderModel::field('*')->where(formatWhere($tuis))->count();
        ...
        //退款
        $tui = [];
        $tui['refund_time']=['between',[$beginToday,$endToday]];
        $tui['is_pay']=1;
        $tui['is_refund']=1;
        
        $tuiday = OrderModel::field('*')->where(formatWhere($tui))->count();
