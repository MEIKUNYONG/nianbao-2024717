...
class Index extends BaseController
{
    ...
    protected $middleware = [ 
    	ApiMiddleware::class . ':api' 	=> ['except' 	=> ['index','config','csconfig','geturlscheme'] ]
    ];
    ...
    public function index(){
        ...
        $data = ['code' =>200,'data'=>$response,'msg' => '获取成功'];
        return json($data);
    ...
    public function config()
    {
        ...
        $list['is_qiang']=$config['is_qiang'];
        $list['normalfee']=$config['normalfee'];
        
        $list['is_yc']=$config['is_yc'];
        $list['service_telephone']=$config['service_telephone'];
        
        $list['shoptitle']=$config['shoptitle']?$config['shoptitle']:"服务费用";
        
        $list['ycshoptitle']=$config['ycshoptitle']?$config['ycshoptitle']:"服务费用";
        
        $min_price=$config['min_price'];
        
        $max_price=$config['max_price'];
        
        $list['newprice']=rand($min_price, $max_price);
        
        $ycmin_price=$config['ycmin_price'];
        
        $ycmax_price=$config['ycmax_price'];
        
        $list['ycnewprice']=rand($ycmin_price, $ycmax_price);
    ...
    public function enterprise()
    {
        ...
        $status=$data['status']?$data['status']:0;//中间表id
        ...
        if($is_report==1){
            ...
            $list['db_img']=$config['db_img'];
            $list['title']=$config['shoptitle']?$config['shoptitle']:"服务费用";
            // 获取今年的年份
            $thisYear = date('Y');
            $list['thisyear']=$thisYear;
            ...
            $newprice=rand($min_price, $max_price);
            
            $ycmin_price=$config['ycmin_price'];
        
            $ycmax_price=$config['ycmax_price'];
                
            $ycnewprice=rand($ycmin_price, $ycmax_price);
            
            if($status==1){
                $pricezong=$ycnewprice;
            }else{
                $pricezong=$newprice;
            }
            ...
            if($is_special==1){
                ...
                $list['price']=$pricezong?$pricezong:$config['specialfee'];
                $list['agreement']=$config['special_content'];
                
                $list['prompt']=$config['specialprompt'];
            }else{
                
                $list['price']=$pricezong?$pricezong:$config['normalfee'];
    ...
    */
    ...
    public function getmo_information()
    {
        ...
        if($status==1){
            $list['title']=$config['ycshoptitle']?$config['ycshoptitle']:"服务费用";
        }else{
            $list['title']=$config['shoptitle']?$config['shoptitle']:"服务费用";
        }
