<?php /*a:2:{s:61:"/www/wwwroot/tieie.xyaa.vip/app/admin/view/remarks/index.html";i:1712244469;s:64:"/www/wwwroot/tieie.xyaa.vip/app/admin/view/common/container.html";i:1693629258;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<el-table :row-class-name="rowClass" @selection-change="selection"  @row-click="handleRowClick"  row-key="id"  :header-cell-style="{ background: '#eef1f6', color: '#606266' }" @sort-change='sortChange' v-loading="loading"  ref="multipleTable" border class="eltable" :data="list"  style="width: 100%">
	...
	<el-table-column align="center"  property="remarks_people"  label="备注人"  width="">
	</el-table-column>
	<el-table-column align="center"  property="name"  label="备注人系统账号昵称"  width="">
	</el-table-column>
	<el-table-column align="center"  property="remarks_content"  label="备注内容"  width="">
	</el-table-column>

	<el-table-column align="center"  property="create_time"  label="备注时间"  width="">
		<template slot-scope="scope">
			{{parseTime(scope.row.create_time)}}
		</template>
	</el-table-column>
	
	<el-table-column align="center"  property="update_time"  label="最后修改时间"  width="">
		<template slot-scope="scope">
	...
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			...
			<div v-if="scope.row.id">
                ...
                <span v-if="(<?php echo session("admin.user_id"); ?>==scope.row.admin_id) &&  (<?php echo session("admin.user_id"); ?>!=1)" >
    				<el-button v-if="checkPermission('/admin/Remarks/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
    				<el-button v-if="checkPermission('/admin/Remarks/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
				</span>
				
				<span v-if="(<?php echo session("admin.user_id"); ?>==1) " >
    				<el-button v-if="checkPermission('/admin/Remarks/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
    				<el-button v-if="checkPermission('/admin/Remarks/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
				</span>
...
<script src="/components/admin/remarks/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/remarks/update.js?v=<?php echo rand(1000,9999)?>"></script>
...
new Vue({
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
			    {name:'添加',color:'success',access:'/admin/Remarks/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Remarks/index',param).then(res => {
				if(res.data.status == 200){
					this.list = res.data.data.data
					this.page_data.total = res.data.data.total
					this.sum_amount = res.data.sum_amount
					this.admin_id = res.data.data.admin_id
					this.loading = false
				}else{
					this.$message.error(res.data.msg);
				}
		...
		updateExt(row,field){
			if(row.id){
				axios.post(base_url + '/Remarks/updateExt',{id:row.id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Remarks/getUpdateInfo',{id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/Remarks/delete',{id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
