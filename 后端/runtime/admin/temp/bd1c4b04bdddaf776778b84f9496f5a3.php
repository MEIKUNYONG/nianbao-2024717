<?php /*a:2:{s:53:"/www/wwwroot/nianbao/app/admin/view/report/index.html";i:1687768005;s:57:"/www/wwwroot/nianbao/app/admin/view/common/container.html";i:1693629258;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<!--	<el-table-column align="center" type = '' property="id"  label="编号"  width="70">
	...
	<el-table-column align="center"  property="report_year"  label="年报年份"  width="">
	</el-table-column>

	<el-table-column align="center"  property="is_report"  label="年报状态"  width="">
	    <template slot-scope="scope">
			<div v-if="scope.row.is_report==1">
			    已年报
			</div>
			<div v-if="scope.row.is_report==0">
			    未年报
			</div>
		</template>
	</el-table-column>
	...
	<!--<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.id">
			    <el-button v-if="checkPermission('/admin/Enterprise/report.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini"  type="warning" @click="report(scope.row)" >年报状态</el-button>
...
new Vue({
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Report/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.id){
				axios.post(base_url + '/Report/updateExt',{id:row.id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Report/getUpdateInfo',{id:id}).then(res => {
				if(res.data.status == 200){
