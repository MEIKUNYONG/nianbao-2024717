...
class Uploadfile extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$orderby = ($sort && $order) ? $sort.' '.$order : 'uploadfile_id desc';

			$res = UploadfileModel::where(formatWhere($where))->field($field)->order($orderby)->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
