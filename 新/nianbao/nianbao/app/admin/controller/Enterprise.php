...
class Enterprise extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$res = EnterpriseModel::where(formatWhere($where))->field($field)->order($orderby)->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	function delete(){
		...
		EnterpriseModel::destroy(['id'=>explode(',',$idx)],true);
		return json(['status'=>200,'msg'=>'操作成功']);
	}
