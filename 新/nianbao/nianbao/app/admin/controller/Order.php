...
class Order extends Admin {
	...
	function index(){
		if (!$this->request->isPost()){
			...
			$where = [];
			$where['u.id'] = $this->request->post('id', '', 'serach_in');
			$where['u.order_no'] = $this->request->post('order_no', '', 'serach_in');
		
			$where['u.mobile'] = $this->request->post('mobile', '', 'serach_in');
			$where['u.is_declaration'] = $this->request->post('is_declaration', '', 'serach_in');
			$where['u.is_success'] = $this->request->post('is_success', '', 'serach_in');
			
			$where['ub.legal_person'] = $this->request->post('legal_person', '', 'serach_in');
			
			
		    
			$where['u.is_pay'] = 1;
			
			$is_status = $this->request->post('is_status', '', 'serach_in');
			if($is_status=="1"){
			    $where['u.is_pay'] = 1;
			    $where['u.status'] = 0;
			}elseif($is_status=="2"){
			...
			$where['u.pay_time'] = ['between',[$pay_time[0],$pay_time[1]]];

			$field = 'u.*,ub.legal_person';
			...
			$res = OrderModel::alias('u')->join("enterprise ub", 'u.enterprise_id=ub.id', 'left')->where(formatWhere($where))->field($field)->order($orderby)->paginate(['list_rows'=>$limit,'page'=>$page])->toArray();
	...
	public function init()  // 支付配置初始化
    {
	    ...
	    $cert_path=app()->getRootPath().'public'.$configs['cert_path'];
	    $key_path=app()->getRootPath().'public'.$configs['key_path'];
	    if (!$cert_path || !$key_path) {
	        throw new ValidateException ('请先配置微信证书');
	    }
	    $config = [
	...
	public function refund(){
	    ...
	    if($order['pay_type']==1){
            ...
            if ($result['result_code'] == 'SUCCESS'){
                ...
                return json(['status'=>201,'msg'=>$result['err_code_des']]);
